...
describe("Network", () => {
    ...
    test("Simple One to One Communication", () => {
        ...
        const bobToMary = bob.sendMessage(new Message().to(mary.id).containing('Hello Mary From Bob'));
        const maryToAlice = mary.sendMessage(new Message().to(alice.id).containing('Hello Alice From Mary'));
        const aliceToMary = alice.sendMessage(new Message().to(mary.id).containing('Hello There Mary From Alice'));
        ...
        [maryReceived1, maryReceived2, aliceReceived].forEach(r => expect(r).toBeDefined());
