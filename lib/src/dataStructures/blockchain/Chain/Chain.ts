...
class Chain {
     ...
     */
    addTransaction(transaction: string): this {
    ...
    testMiningOfNextBlock(): boolean {
        ...
        if (passed) {
            ...
            this.nextBlock = new Block(this.blocks.length, this.blocks[this.blocks.length - 1].hash);
        }
        return passed;
    }
